...
test('webhook: assinatura inválida → 400', async () => {
  const res = mockRes();
  await fns.stripeWebhook(
    { rawBody: Buffer.from('{}'), headers: { 'stripe-signature': 'bad' } },
    res
  );
...
test('RGPD: admin elimina em cascata (perfil/subcoleções/chat/marcações/Auth) + auditoria', async () => {
  ...
  await db
    .collection('agendamentosNutri')
    .doc(uid + '_ag')
    .set({ uid, data: '2099-01-01', hora: '09:00' });
  ...
  assert.strictEqual(
    (await db.collection('agendamentosNutri').where('uid', '==', uid).get()).size,
    0
  );
...
test('assignClienteId: idempotente — não reatribui se já tiver clienteId', async () => {
  ...
  const wrapped = fft.wrap(fns.assignClienteId);
  await wrapped({
    data: fft.firestore.makeDocumentSnapshot(
      { estado: 1, clienteId: 'C9999', seq: 9999 },
      `users/${uid}`
    ),
  });
...
test('syncBusySlots: cancelamento remove a hora; eliminar a última hora apaga o doc', async () => {
  ...
  assert.deepStrictEqual((await db.collection('busySlots').doc(data).get()).data().horas, [
    '11:00',
  ]);
